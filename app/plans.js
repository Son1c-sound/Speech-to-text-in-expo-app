...
const SubscriptionPurchase = () => {
  ...
  const getSubscriptionPackage = async () => {
    try {
      ...
      if (offerings.current && offerings.current.availablePackages.length !== 0) {
        setSubscription(offerings.current.availablePackages[0]); // Get first package
      }
    } catch (error) {
      console.error('Error getting subscription package:', error);
      Alert.alert('Error', 'Could not load subscription package');
    } finally {
      setLoading(false);
    }
  };
  ...
  const handleSubscription = async () => {
    try {
      ...
      if (customerInfo.entitlements.active.premium) {
        ...
        // Handle successful subscription (e.g., unlock features, update UI)
      }
    } catch (error) {
  ...
  if (!subscription) {
    return (
      <View style={styles.container}>
        <Text>No subscription package available</Text>
      </View>
    );
  }
    ...
    <View style={styles.container}>
      ...
      >
        <Text style={styles.buttonText}>
          Subscribe for {subscription.product.priceString}
        </Text>
